...
class PrInlineMacro < Asciidoctor::Extensions::InlineMacroProcessor
  ...
  def process parent, target, attrs
    pr = target
    target = "https://github.com/wgsl-analyzer/wgsl-analyzer/pull/#{pr}"
...
class CommitInlineMacro < Asciidoctor::Extensions::InlineMacroProcessor
  ...
  def process parent, target, attrs
    hash = target
    target = "https://github.com/wgsl-analyzer/wgsl-analyzer/commit/#{hash}"
    parent.document.register :links, target
...
class ReleaseInlineMacro < Asciidoctor::Extensions::InlineMacroProcessor
  ...
  def process parent, target, attrs
    date = target
    target = "https://github.com/wgsl-analyzer/wgsl-analyzer/releases/#{date}"
    parent.document.register :links, target
